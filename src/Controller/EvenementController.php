...
final class EvenementController extends AbstractController
{
    ...
    {
        $categories = ["culturel", "festif","sportif","associatif"];
    ...
    #[Route('/evenement/categorie/{categorie}', name: 'app_categorie')]
    public function getCategorie(string $categorie): Response{
        $evenements = $this->getEvenements();
        $evenementsFilter =array_filter($evenements, function ($event) use ($categorie){
            return strtolower($event['categorie']) && $event['categorie'] == $categorie;
        }) ?? null;
        if (!$evenementsFilter) {
            throw new NotFoundHttpException("La categorie $categorie n'existe pas ou ne contient aucun evenement.");
        }

//        echo '<pre>';
//        var_dump($evenementsFilter);
//        echo '</pre>';
        return $this->render('evenement/categorie/index.html.twig', [
            'categorie' => $categorie,
            'evenements' => $evenementsFilter,
        ]);
    }

    #[Route('/evenement/{id}', name: 'app_id')]
    public function getInfoLivre(string $id): Response{
        $evenements = $this->getEvenements();
        $evenementsFilter =array_filter($evenements, function ($event) use ($id){
            return isset($event['id']) && $event['id'] == $id;
        })?? null;
        if (!$evenementsFilter) {
            throw new NotFoundHttpException("Cette evenement  n'existe pas.");
        }
        $evenement = array_shift($evenementsFilter);
//        echo '<pre>';
//        var_dump($evenement);
//        echo '</pre>';
        return $this->render('evenement/id/index.html.twig', [
            'evenement' => $evenement,
        ]);
    }
